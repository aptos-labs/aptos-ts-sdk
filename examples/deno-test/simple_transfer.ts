 ...
 * to create accounts, fund them, and transfer between them.
 */
...
const APTOS_NETWORK: Network = NetworkToNetworkName[Deno.env.get("APTOS_NETWORK") ?? "devnet"];
...
const balance = async (aptos: Aptos, name: string, address: AccountAddress): Promise<number> => {
  ...
  const [balanceResult] = await aptos.viewJson<[number]>({ payload });

  console.log(`${name}'s balance is: ${balanceResult}`);
  return Number(balanceResult);
};

const example = async () => {
  console.log("Deno Runtime Test: Creating accounts, funding them, and transferring between them.");
