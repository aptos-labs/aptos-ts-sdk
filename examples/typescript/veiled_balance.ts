...
const sendAndWaitTx = async (transaction: AnyRawTransaction, signer: Account): Promise<CommittedTransactionResponse> => {
...
const getVeiledBalances = async (name: string, accountAddress: AccountAddress, privateKey: TwistedEd25519PrivateKey) => {
...
async function example() {
  ...
  await Promise.all([
    sendAndWaitTx(aliceRegisterVBTx, alice),
    sendAndWaitTx(bobRegisterVBTx, bob),
  ])
