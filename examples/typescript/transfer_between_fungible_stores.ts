...
async function createSecondaryStore(admin: Account, user: Account, metadata: AccountAddress): Promise<string> {
  ...
  const senderAuthenticator = cedra.transaction.sign({ signer: user, transaction });
  const pendingTxn = await cedra.transaction.submit.simple({ transaction, senderAuthenticator });
...
async function main() {
  try {
    ...
    // Fund accounts
    await Promise.all([
      cedra.fundAccount({ accountAddress: alice.accountAddress, amount: 100_000_000 }),
      cedra.fundAccount({ accountAddress: bob.accountAddress, amount: 100_000_000 }),
      cedra.fundAccount({ accountAddress: charlie.accountAddress, amount: 100_000_000 }),
    ]);
    ...
    const transferToSecondaryTxn = await cedra.transferFungibleAssetBetweenStores({
      sender: bob,
      fromStore: bobPrimaryStoreAddress,
      toStore: bobSecondaryStore,
      amount: 800_000_000,
    });
    const transferToSecondaryResponse = await cedra.signAndSubmitTransaction({
      signer: bob,
      transaction: transferToSecondaryTxn,
    });
    await cedra.waitForTransaction({ transactionHash: transferToSecondaryResponse.hash });
    ...
    const transferBetweenSecondaryTxn = await cedra.transferFungibleAssetBetweenStores({
      sender: bob,
      fromStore: bobSecondaryStore,
      toStore: charlieSecondaryStore,
      amount: 600_000_000,
    });
    const transferBetweenSecondaryResponse = await cedra.signAndSubmitTransaction({
      signer: bob,
      transaction: transferBetweenSecondaryTxn,
    });
    await cedra.waitForTransaction({ transactionHash: transferBetweenSecondaryResponse.hash });
    ...
    const transferFromSecondaryTxn = await cedra.transferFungibleAssetBetweenStores({
      sender: charlie,
      fromStore: charlieSecondaryStore,
      toStore: bobPrimaryStoreAddress,
      amount: 350_000_000,
    });
    const transferFromSecondaryResponse = await cedra.signAndSubmitTransaction({
      signer: charlie,
      transaction: transferFromSecondaryTxn,
    });
    await cedra.waitForTransaction({ transactionHash: transferFromSecondaryResponse.hash });
