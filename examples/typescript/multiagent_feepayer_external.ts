...
const example = async () => {
  ...
  const simpleTransaction = await aptos.transaction.build.multiAgent({
  ...
  // TODO: It looks like there's a bug with multi-agent where the secondary signer must know the fee payer
  // This line should be able to be removed once fixed.
  simpleTransaction.feePayerAddress = feePayer.address();
  ...
  console.log("\n=== Signing ===\n");
  const userAuthenticator = user.signTransactionWithAuthenticator(simpleTransaction);
  console.log(`Retrieved user authenticator: ${JSON.stringify(userAuthenticator)}`);

  const adminBytes = admin.sign(simpleTransaction.bcsToBytes());
  ...
  const feePayerBytes = feePayer.sign(simpleTransaction.bcsToBytes());
