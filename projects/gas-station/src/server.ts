...
app.post("/signAndSubmit", async (req: Request, res: Response) => {
  try {
    ...
    const deserializedSenderAuth = AccountAuthenticator.deserialize(
      new Deserializer(Uint8Array.from(senderAuthenticator)),
    );
    ...
    const response = await aptos.transaction.submit.simple(signedTxnInput);
