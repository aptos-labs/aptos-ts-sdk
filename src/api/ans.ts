...
/**
 * Retrieve the owner address of a specified domain or subdomain from the contract.
 * 
 ...
 * 
 * async function runExample() {
 *   // Retrieve the expiration time for the subdomain "test.aptos"
 *   const exp = await aptos.getExpiration({ name: "test.aptos" });
 * 
 *   // Log the expiration date if it exists
 *   if (exp) {
 *     console.log(new Date(exp)); // Outputs the expiration date
 *   } else {
 *     console.log("The domain does not exist or has no expiration.");
 *   }
 * }
 * runExample().catch(console.error);
 */
...
/**
 * Retrieve the target address of a domain or subdomain name for on-chain use. 
 * The target address can point to addresses that do not own the name.
 *
 ...
 *   
 *   console.log(targetAddr); // Logs the target address or undefined if not found
 * }
 * runExample().catch(console.error);
 */
...
/**
 * Sets the target address of a domain or subdomain name, pointing it to a specified address for on-chain use. 
 * Note that the target address can be set to addresses that are not the owner of the name.
 *
 ...
 * @param args.address - An AccountAddressInput specifying the address to set the domain or subdomain to.
 ...
 * 
 * async function runExample() {
 *   // Set the target address for the domain "test.aptos"
...
/**
 * Retrieve the primary name for an account. An account can have multiple names that target it, but only a single name can be primary. An account may not have a primary name.
 *
 * @param args - The arguments for the function.
 ...
 *   console.log(name); // Outputs the primary name or undefined
 * }
 * runExample().catch(console.error);
 */
...
/**
 * Sets the primary name for the specified sender account. An account can have multiple names, but only one primary name can be set at a time.
 * 
 ...
 * 
 * async function runExample() {
 *   const sender = Account.generate(); // Replace with your own account
 * 
 ...
 * 
 * This function allows users to register a new domain or subdomain name with specific expiration policies. 
 * 
 * @param args.sender - The sender account.
 ...
 * async function runExample() {
 *   const sender = Account.generate(); // Replace with a real account
 *   const domainName = "test"; // Replace with a real domain name
 * 
 ...
 * 
 * @returns A promise of an ANSName or undefined if the name is not found or inactive.
 * 
 * @example
 * ```typescript
 * import { Aptos, AptosConfig, Network } from "@aptos-labs/ts-sdk";
 ...
 *   const name = await aptos.getName({ name: "test.aptos.apt" }); // replace with a real name
 *   console.log(name);
 * }
 * runExample().catch(console.error);
 * ```
 ...
 *       orderBy: "name", // specify the order to sort the names
 *     },
 *   });
 * 
 ...
 * @param args.accountAddress - The address of the account to retrieve domain names for.
 * @param args.options.offset - Optional, the offset to start from when fetching names.
 * @param args.options.limit - Optional, the number of names to fetch per request.
 * @param args.options.orderBy - Optional, the order to sort the names by.
 * @param args.options.where - Optional, additional filters to apply to the query.
 * 
 ...
 * 
 * async function runExample() {
 *   // Fetching all top-level domain names for a given account
 *   const accountAddress = "0x1"; // replace with a real account address
 *   const domains = await aptos.getAccountDomains({
 *     accountAddress,
 *     options: {
 *       limit: 10, // specify the number of names to fetch
 *       orderBy: "name", // specify the order by which to sort names
 *     },
 *   });
 * 
 *   console.log(domains);
...
/**
 * Fetches all subdomain names for a specified account address.
 * 
 * @param args - The arguments for fetching subdomains.
 * @param args.accountAddress - The account address to retrieve subdomain names for.
 ...
 * 
 * async function runExample() {
 *   // Fetching subdomains for a specific account address
 ...
 * 
 * async function runExample() {
 *   // Fetching subdomains for the domain "test.apt"
 *   const subdomains = await aptos.getDomainSubdomains({
 *     domain: "test.apt", // specify the domain name
 *     options: {
 ...
 *   });
 * 
 *   console.log(subdomains); // Output the fetched subdomains
 * }
 * runExample().catch(console.error);
 * ```
 */
