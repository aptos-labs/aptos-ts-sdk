...
export type EntryFunctionArgumentABI = {
    types: Array<Kind>,
    comment?: string,
}

const TypeClasses = { Bool, U8, U16, U32, U64, U128, U256, AccountAddress, MoveString, MoveVector, MoveOption, TypeTagStruct };
...
function metaclassBuilder(className: string, typeTags: Array<TypeTag>, suppliedFieldNames?: Array<string>): string {
    ...
    const classFields = getClassArgTypes(typeTags);
    ...
    classFields.forEach((classField, i) => {
        const argComment = ` // ${classField.annotation}`;
        lines.push(`    public readonly ${fieldNames[i]}: ${classField.kindString}; ${argComment}\n`);
    });
    lines.push('\n');
    lines.push(`    constructor(`);
    classFields.forEach((classField, i) => {
        lines.push(`        ${fieldNames[i]}: ${classField.kindString},`);
    });
    // return lines.join('\n');
    lines.push('}');
    return '';
...
function getClassArgTypes(typeTags: Array<TypeTag>): Array<BCSClassAnnotated> {
    const bcsClassesAnnotated = new Array<BCSClassAnnotated>();
    typeTags.forEach((typeTag) => {
        ...
        if (kindArray.length > 1) {
            ...
            if (kindArray[kindArray.length - 2] == AccountAddress.kind) {
                console.log('asdfsdafjsdfasdfhaskdfhasdkfhasdkjhfsakjfh')
                console.log('asdfsdafjsdfasdfhaskdfhasdkfhasdkjhfsakjfh')
                console.log('asdfsdafjsdfasdfhaskdfhasdkfhasdkjhfsakjfh')
                console.log('asdfsdafjsdfasdfhaskdfhasdkfhasdkjhfsakjfh')
                console.log('asdfsdafjsdfasdfhaskdfhasdkfhasdkjhfsakjfh')
