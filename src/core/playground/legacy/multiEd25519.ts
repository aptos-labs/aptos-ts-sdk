...
export class LegacyMultiEd25519PublicKey {
  ...
  constructor(args: { publicKeys: Ed25519PublicKey[]; threshold: number }) {
    ...
    if (
      publicKeys.length > LegacyMultiEd25519PublicKey.MAX_KEYS ||
      publicKeys.length < LegacyMultiEd25519PublicKey.MIN_KEYS
    ) {
      throw new Error(
        `Must have between ${LegacyMultiEd25519PublicKey.MIN_KEYS} and ` +
          `${LegacyMultiEd25519PublicKey.MAX_KEYS} public keys, inclusive`,
      );
    }

    // Validate threshold: must be between 1 and the number of public keys, inclusive
    if (threshold < LegacyMultiEd25519PublicKey.MIN_THRESHOLD || threshold > publicKeys.length) {
      throw new Error(
        `Threshold must be between ${LegacyMultiEd25519PublicKey.MIN_THRESHOLD} and ${publicKeys.length}, inclusive`,
      );
    }
  ...
  toUint8Array(): Uint8Array {
    ...
    this.publicKeys.forEach((k: Ed25519PublicKey, i: number) => {
      bytes.set(bcsToBytes(k), i * Ed25519PublicKey.LENGTH);
    });
  ...
  verifySignature({ message, signature }: { message: HexInput; signature: LegacyMultiEd25519Signature }): boolean {
    ...
    for (let i = 0; i < indices.length; i += 1) {
      ...
      if (!publicKey.verifySignature(message, signature.signatures[i])) {
        return false;
      }
    }
    return true;
  ...
  static deserialize(deserializer: Deserializer): LegacyMultiEd25519PublicKey {
    ...
    return new LegacyMultiEd25519PublicKey({ publicKeys: keys, threshold });
  }
}
...
export class LegacyMultiEd25519Signature {
  ...
  toUint8Array(): Uint8Array {
    const bytes = new Uint8Array(
      this.signatures.length * Ed25519Signature.LENGTH + LegacyMultiEd25519Signature.BITMAP_LEN,
    );
    this.signatures.forEach((k: Ed25519Signature, i: number) => {
      bytes.set(bcsToBytes(k), i * Ed25519Signature.LENGTH);
    });
  ...
  static createBitmap(args: { bits: number[] }): Uint8Array {
    ...
    bits.forEach((bit: number) => {
      if (bit >= LegacyMultiEd25519Signature.MAX_SIGNATURES_SUPPORTED) {
        throw new Error(
          `Cannot have a signature larger than ${LegacyMultiEd25519Signature.MAX_SIGNATURES_SUPPORTED - 1}.`,
        );
      }
  ...
  static deserialize(deserializer: Deserializer): LegacyMultiEd25519Signature {
    const bytes = deserializer.deserializeBytes();
    ...
    return new LegacyMultiEd25519Signature({ signatures, bitmap });
  }
}
