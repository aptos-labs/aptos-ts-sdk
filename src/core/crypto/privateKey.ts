    ...
    if (typeof value === "string") {
      if (!strict && !value.startsWith(aip80Prefix)) {
        ...
        if (strict !== false) {
          ...
          console.warn(
            "[Cedra SDK] It is recommended that private keys are AIP-80 compliant (https://github.com/cedra-foundation/AIPs/blob/main/aips/aip-80.md). You can fix the private key by formatting it with `PrivateKey.formatPrivateKey(privateKey: string, type: 'ed25519' | 'secp256k1'): string`.",
          );
        }
