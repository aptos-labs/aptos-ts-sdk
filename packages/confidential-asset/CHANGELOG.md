...
- **Fiat-Shamir batching challenge now binds the prover response σ (aptos-core #19711).** `sigmaProtocolFiatShamir` appends `BCS(sigmas)` to the `β` transcript (`SHA2-512(seed || 0x01 || BCS(σ))`), matching on-chain `sigma_protocol_fiat_shamir::fiat_shamir`. `e` is still independent of σ so honest proving is unchanged. Omitting σ from `β` let a malicious prover choose the response after seeing the batching coefficients and cancel individual verification equations in the aggregated MSM. The prover continues to pass an empty σ vector (it only needs `e`); the SDK verifier passes the proof response so derived `β` values match Move. New regression tests pin: changing σ changes `β` but not `e`; changing `A` or the statement changes both.
- **`TwistedEd25519PublicKey` rejects the identity point and non-canonical Ristretto255 encodings**, matching on-chain `new_pubkey_from_bytes` (confidential-asset v1.1.2). Encrypting under the identity collapses `D = r·𝒪 = 𝒪`, which makes auditor ciphertexts undecryptable and degenerates the auditor-binding term in the transfer proof. New `TwistedEd25519PublicKey.isIdentity()` helper for the canonical 32-zero-byte encoding.
