 ...
 */
export type PluginConfig<TSubmitter = TransactionSubmitter> = {
  /**
   ...
   */
  TRANSACTION_SUBMITTER?: TSubmitter;
...
export type PluginSettings<TSubmitter = TransactionSubmitter> = Omit<PluginConfig<TSubmitter>, "IGNORE_TRANSACTION_SUBMITTER">;
 ...
 */
/**
 * Arguments passed to a {@link TransactionSubmitter} implementation.
 */
export type TransactionSubmitRequest<TAptosConfig = any, TSubmitArgs = any> =
  {
    aptosConfig: TAptosConfig;
  } & TSubmitArgs;

export interface TransactionSubmitter<TAptosConfig = any, TSubmitArgs = any, TResponse = unknown> {
  /**
   ...
   */
  submitTransaction(
    args: TransactionSubmitRequest<TAptosConfig, TSubmitArgs>,
  ): Promise<TResponse>;
}
...
/**
 * Legacy alias for {@link ClientRequest} maintained for backward compatibility with external consumers.
 */
export type AptosClientRequest<Req = unknown> = ClientRequest<Req>;

/**
 * Legacy alias for {@link ClientResponse} maintained for backward compatibility with external consumers.
 */
export type AptosClientResponse<Res = unknown> = ClientResponse<Res>;
