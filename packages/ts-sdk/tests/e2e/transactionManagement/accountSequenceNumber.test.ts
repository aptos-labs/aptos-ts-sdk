...
import * as AccountQueries from "@aptos-labs/ts-api/internal/account";
import { AccountSequenceNumber } from "@aptos-labs/ts-transactions/management/accountSequenceNumber";
import { getAptosClient } from "../helper";

const { aptos, config: aptosConfig } = getAptosClient();
const accountQueriesMockTarget = AccountQueries as Record<string, (...args: any[]) => unknown>;

const account = Account.generate();

const accountSequenceNumber = new AccountSequenceNumber(aptosConfig, account, 30, 100, 10);
let getAccountInfoSpy: ReturnType<typeof vi.spyOn>;
...
describe("account sequence number", () => {
  ...
  beforeEach(() => {
    getAccountInfoSpy = vi.spyOn(accountQueriesMockTarget, "getInfo");
  });
