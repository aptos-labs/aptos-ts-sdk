...
import { memoizeAsync, memoize } from "@aptos-labs/ts-core/utils";

describe("memoize", () => {
  describe("memoizeAsync", () => {
    // Define an asynchronous function to mock
    const asyncFunction = vi.fn(async (arg) => {
      // Simulate async work without relying on real timers to avoid hanging fake timers.
      await Promise.resolve();
      return arg;
    });

    beforeEach(() => {
      // Use fake timers to stabilize timer-based memoization assertions.
      vi.useFakeTimers();
    });
