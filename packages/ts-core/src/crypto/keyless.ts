...
import { Deserializer, Serializable, Serializer } from "@aptos-labs/ts-bcs";
...
import { memoizeAsync, base64UrlToBytes, getErrorMessage, nowInSeconds } from "../utils";
...
import { generateSigningMessage } from "@aptos-labs/ts-transactions/transactionBuilder/signingMessage";
import { ProjPointType } from "@noble/curves/abstract/weierstrass";
import { Fp2 } from "@noble/curves/abstract/tower";
import type { AptosConfigLike, GetAptosRequestOptions } from "../network/types";
import { getRegisteredNetworkClient } from "../network/client";

function callFullNode<Req extends {}, Res extends {}>(options: GetAptosRequestOptions) {
  const { getAptosFullNode } = getRegisteredNetworkClient();
  return getAptosFullNode<Req, Res>(options);
}
...
async function getKeylessConfigurationResource(args: {
  ...
  try {
    const { data } = await callFullNode<{}, MoveResource<KeylessConfigurationResponse>>({
      aptosConfig,
...
async function getGroth16VerificationKeyResource(args: {
  ...
  try {
    const { data } = await callFullNode<{}, MoveResource<Groth16VerificationKeyResponse>>({
      aptosConfig,
...
export async function getKeylessJWKs(args: {
  ...
  if (!jwkAddr) {
    ...
    const { data } = await callFullNode<{}, MoveResource<PatchedJWKsResponse>>({
      aptosConfig,
