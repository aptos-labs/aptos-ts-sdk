...
import { AccountAddress, AccountAddressInput, Hex, PublicKey } from "@aptos-labs/ts-core";
import {
  AnyPublicKey,
  AnySignature,
  KeylessPublicKey,
...
import { NetworkToChainId } from "@aptos-labs/ts-types";
import { normalizeBundle } from "@aptos-labs/ts-bcs";
import { AptosConfig } from "@aptos-labs/ts-client";
import { getTransactionPluginHost } from "../plugins/host";
import {
...
import { memoizeAsync } from "@aptos-labs/ts-core";
...
export async function generateRawTransaction(args: {
  ...
  const pluginHost = getTransactionPluginHost();
  ...
  const getChainId = async () => {
    ...
    const info = await pluginHost.getLedgerInfo({ aptosConfig });
  ...
  const getGasUnitPrice = async () => {
    ...
    const estimation = await pluginHost.getGasPriceEstimation({ aptosConfig });
  ...
  const getSequenceNumberForAny = async () => {
    const getSequenceNumber = async () => {
      ...
      return (
        await pluginHost.getAccountInfo({
          aptosConfig,
