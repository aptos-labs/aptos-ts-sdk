...
describe("transaction submission", () => {
  ...
  describe("publish move module", () => {
    describe("with hex encoded module", () => {
    ...
    describe.only("with file reader", () => {
      // current working directory - the root folder of this repo
      const cwd = process.cwd();
      // target directory - current working directory + movepackage folder
      const modulePath = path.join(cwd, "tests/move/transfer");

      const metadataBytes = fs.readFileSync(path.join(modulePath, "build", "transaction_test", "package-metadata.bcs"));
      const byteCode = fs.readFileSync(
        path.join(modulePath, "build", "transaction_test", "bytecode_modules", "transfer.mv"),
      );

      it("it generates a publish move module transaction successfully ", async () => {
        const transaction = await aptos.publishPackageTransaction({
          account: contractPublisherAccount.accountAddress.toString(),
          metadataBytes,
          moduleBytecode: [byteCode],
        });
        expect(transaction.rawTransaction instanceof Uint8Array).toBeTruthy();
      ...
      test("it submits a publish move module transaction successfully", async () => {
        ...
        const response = await aptos.signAndSubmitTransaction({
          signer: contractPublisherAccount,
          transaction,
        });
        ...
        const accountModules = await aptos.getAccountModules({
          accountAddress: contractPublisherAccount.accountAddress.toString(),
        });
        expect(accountModules[0].bytecode).toEqual(`0x${byteCode}`);
      });
    });
