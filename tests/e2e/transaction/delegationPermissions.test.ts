...
import { FungibleAssetPermission, GasPermission, NFTPermission } from "../../../src/types/permissions";
...
describe("transaction submission", () => {
  ...
  beforeEach(async () => {
    ...
    abstractAccount = AbstractedAccount.fromPermissionedSigner({
      signer: Account.fromPrivateKey({ privateKey: permissionedAccount1.privateKey }),
    });
  ...
  test("Able to grant permissions for NFTs", async () => {
    ...
    await signSubmitAndWait({
      ...
      transaction: await aptos.permissions.requestPermissions({
        ...
        delegationPublicKey: permissionedAccount1.publicKey,
        permissions: [
          NFTPermission.from({ assetAddress: nftAddress, capabilities: { transfer: true, mutate: false } }),
        ],
      }),
    });
