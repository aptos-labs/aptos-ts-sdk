...
describe("transactionWorker", () => {
  beforeAll(async () => {
    await cedra.fundAccount({ accountAddress: sender.accountAddress, amount: 1000000000 });
  });
  ...
  test(
    ...
    async () => {
      ...
      const transactionWorker = new TransactionWorker(cedraConfig, sender);
  ...
  test(
    ...
    (done) => {
      ...
      setTimeout(async () => {
        transactionWorker.stop();
        const accountData = await cedra.getAccountInfo({ accountAddress: sender.accountAddress });
        ...
        const txns = await cedra.getAccountTransactions({ accountAddress: sender.accountAddress });
