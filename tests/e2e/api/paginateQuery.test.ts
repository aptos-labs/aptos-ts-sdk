...
describe("PaginateQuery", () => {
  ...
  test("it should paginate correctly on indexer queries", async () => {
    ...
    const firstTenEvents = await cedra.getEvents({ options: { limit: 10 } });
    ...
    const onlyNextTwentyEvents = await cedra.getEvents({ options: { offset: 10, limit: 10 } });
  ...
  test("it should paginate correctly on fullnode queries", async () => {
    ...
    const firstTenTxs = await cedra.getTransactions({ options: { offset: 0, limit: 10 } });
    ...
    const onlyNextTwentyTxs = await cedra.getTransactions({ options: { offset: 10, limit: 10 } });
