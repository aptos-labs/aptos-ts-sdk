...
// Raw hex data for lazy deserialization (avoids circular dependency issues at module load time)
const KEYLESS_PROOF_HEX =
...
// Lazy getters to avoid circular dependency issues at module load time
let _cachedProof: ZeroKnowledgeSig | undefined;
let _cachedJwk: MoveJWK | undefined;

function getKeylessProof(): ZeroKnowledgeSig {
  if (!_cachedProof) {
    _cachedProof = ZeroKnowledgeSig.deserialize(new Deserializer(Hex.hexInputToUint8Array(KEYLESS_PROOF_HEX)));
  }
  return _cachedProof;
}

function getKeylessJwk(): MoveJWK {
  if (!_cachedJwk) {
    _cachedJwk = MoveJWK.deserialize(new Deserializer(Hex.hexInputToUint8Array(KEYLESS_JWK_HEX)));
  }
  return _cachedJwk;
}
...
export const keylessTestObject = {
  ...
  get jwk() {
    return getKeylessJwk();
  },
};

// Lazy getter for keylessTestConfig to avoid circular dependency issues
let _cachedKeylessTestConfig: KeylessConfiguration | undefined;
export function getKeylessTestConfig(): KeylessConfiguration {
  if (!_cachedKeylessTestConfig) {
    _cachedKeylessTestConfig = new KeylessConfiguration({
  ...
  return _cachedKeylessTestConfig;
}
