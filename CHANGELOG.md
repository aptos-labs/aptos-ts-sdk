...
- Harden `Move.runCommand` (the `aptos move *` helper used by `Move.compile`/`runTests`/`publish`/etc.) against shell-command-injection on Windows. Now validates every argument against a strict shell-metacharacter denylist (`; | & \` $ < > ( ) * ? ! \\ \n \r \0`) and rejects with a descriptive error before invoking `spawn`, so `shell: true` (still required to launch `npx`'s `.cmd` shim per the CVE-2024-27980 mitigation) only ever sees pre-validated argv. Addresses the CodeQL `js/shell-command-injection-from-environment` finding raised in PR #891 review.
