...
- `LocalNode.start` and `Move.runCommand` no longer spawn `npx` (the `.cmd` shim on Windows) and no longer set `shell: true` on any platform. They now invoke the bundled `npx-cli.js` directly via `process.execPath` (the current Node binary), eliminating the shell-injection sink that CodeQL's `js/shell-command-constructed-from-input` flags. The Windows-only `shell: true` branch — previously kept because Node 18.20.2+ / 20.12.2+ refuses to spawn `.cmd` / `.bat` files without a shell — is removed entirely. The `assertSafeCliArgs` validator is retained as defense-in-depth. Resolution of `npx-cli.js` is handled by a new `resolveNpxCli` helper (exported from `@aptos-labs/ts-sdk/cli`) that searches the standard layouts under `process.execPath` (nodejs.org installers, nvm, fnm, asdf, Windows MSI all covered).
