...
- All SDK requests now send `accept-encoding: identity` by default to work around an upstream bug in `@aptos-labs/aptos-client`'s HTTP/2 path where brotli-compressed bodies are returned undecoded, causing JSON parsing to silently fall back to a raw-bytes string (so e.g. `info.authentication_key` came back `undefined` on devnet). Callers can re-enable compression by setting their own `accept-encoding` in `clientConfig.HEADERS`.
