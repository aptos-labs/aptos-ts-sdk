...
- `aptos.ans.*` queries no longer throw on indexer rows with missing fields. `sanitizeANSName` now returns safe defaults instead of non-null-asserting: `domain` → `"N/A"`, `token_standard` → `"v2"`, `is_primary` → `false`, `registered_address` → `undefined`.
